...
import com.danibrunen.mijuego.actores.Boss;
...
public class Nivel3 extends AbstractScreen {
	...
	public void show() {
		...
		Image imagenFondo = new Image(Principal.MANAGER.get("fondo3.png", Texture.class));
		imagenFondo.setFillParent(true);
		stage.addActor(imagenFondo);
		
		musica = Gdx.audio.newSound(Gdx.files.internal("fondo3.ogg"));
		musica.loop();
		...
		puntuacion = new Puntuacion(new BitmapFont(), "Vida del boss (100): ");
		puntuacion.setPosition(vidaProtagonista.getX(), 15);
		puntuacion.puntuacion = 0;
		stage.addActor(puntuacion);
		
		boss = new Boss();
		boss.setPosition(stage.getWidth(), 50);
		boss.box.x = boss.getX();
		boss.box.y = boss.getY();
		stage.addActor(boss);
	}
	...
	private void comprobarColisiones() {
		...
		for(int i = 0; i < enemigos.size(); i++) {
			...
			if(enemigo.box.overlaps(protagonista.box)) {
				...
				for (int j = 0; j < balas.size(); j++) {
					...
					else if(balas.get(j).box.overlaps(boss.box)) {
						//colision bala-boss
						balas.get(j).remove();
						balas.remove(j);
						Principal.MANAGER.get("hit.mp3", Sound.class).play();
		...
		if(boss.box.overlaps(protagonista.box)) {
			//colision boss-protagonista
			juego.setScreen(juego.FIN);
		}	
	}
